...
usersArr = usersArr.map(user => ({email: user.email, password: hashing(user.password)}))

router.post('/login', (req, res) => {
    const { email, password } = req.body;
    const user = usersArr.find(user => user.email === email && user.password === hashing(password));
